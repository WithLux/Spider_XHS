...
class XHS_Apis():
    ...
    def get_user_notes_by_date(self, user_url: str, cookies_str: str, date_size: int, proxies: dict = None):
        ...
        adjusted_time = datetime.now(tz) - timedelta(days=date_size)
        # 转换成 Unix 时间戳（秒）
    ...
    def get_note_info(self, note_id: str, xsec_token: str, cookies_str: str, proxies: dict = None):
        """
            ...
            :param xsec_token: 你想要获取的笔记的xsec_token
            :param cookies_str: 你的cookies
    ...
    def get_note_out_comment_with_num(self, note_id: str, xsec_token: str, cookies_str: str, num: int,
                                      proxies: dict = None):
        """
            获取笔记的指定数量的一级评论
            :param note_id 笔记的id
        ...
        try:
            while True:
                ...
                if 'cursor' in res_json["data"]:
                    ...
                    break
                note_out_comment_list.extend(comments)
                if len(note_out_comment_list) >= num:
                    return success, msg, note_out_comment_list[:num]
                if len(note_out_comment_list) == 0 or not res_json["data"]["has_more"]:
                    break
        except Exception as e:
            success = False
